...
export const getHtmlInfo = (html: any) => {
  const $ = cheerio.load(html);
  return {
    mainHtml: getMainHtml($),
...
//获取详细的视频信息
export const getVideoInfo = (html: any) => {
  const $ = cheerio.load(html);
 ...
 */
export const getImgInfo = (html: any) => {
  const $ = cheerio.load(html);
...
export const getImgInfoOnly = (html: any) => {
  const $ = cheerio.load(html);
  const original = $('#info > li:nth-child(3) > a').attr('href') || '';
