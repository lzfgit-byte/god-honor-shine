...
<script setup lang="ts">
  ...
  import { nyan, nyanGetType } from '@/sexy-photo/type/sex-photo-type';
  ...
  const getLoliImgs = (option: nyanGetType) => {
    return axiosGet(nyan, option);
  };
  const images = ref();

  const loadImags = (option: nyanGetType) => {
    spinning.value = true;
    option.num = num.value;
    option.r18 = !!r18.value;
    getLoliImgs(option).then((res) => {
